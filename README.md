...
- 内置检查更新、下载安装包及完整性校验；下载后可打开新版安装包，保留本机登录和任务记录。
...
**从 v1.5.0 升级：** 旧版没有内置更新入口，需要先从 Releases 手动安装最新版一次。从 v1.6.1 开始可以在应用内检查后续版本，下载后启动更新安装。Windows 打开安装向导；Mac 打开 DMG 后，将新版拖入“应用程序”。v1.6.3 起应用更名为“Brclio 小红书下载器”，首次更名升级可能同时保留旧名称的 Mac 应用；确认新版正常后可移除旧应用，登录与任务数据继续沿用。Mac v1.7.1 使用 ad hoc 签名，仍没有 Apple Developer ID 签名或公证；Windows 也没有发布者代码签名，系统可能提示验证开发者。系统重启和实际覆盖升级仍需人工验收。
