...
v1.8.12 改进客户端多图复制：勾选后通过系统剪贴板一次复制独立原图文件，保留内容与顺序，可在支持多文件粘贴的应用中一次粘贴。Mac 已用实际 10 图笔记核对一次复制、一次粘贴、全部图片内容和顺序。网页版保持原有行为，详见 [v1.8.12 版本说明](docs/releases/v1.8.12.md)。

v1.8.11 新增客户端“开通会员”：日付 2 元 / 1 天、月付 9.9 元 / 30 天、年付 39.9 元 / 365 天，支持微信与支付宝扫码付款。**付款务必备注软件账号邮箱，核实收款后激活码发送至该邮箱。** 管理员可选择客户邮箱与套餐，生成并发送绑定该账号的激活码，失败可重试原码。详见 [v1.8.11 版本说明](docs/releases/v1.8.11.md) 与 [会员开通操作说明](docs/membership-purchase.md)。
...
**v1.8.3–v1.8.6 的 Mac 升级提醒：** 旧版自带更新助手可能让新应用或恢复的旧应用直接退出，发布新版不会改写旧助手。请下载 [v1.8.12 完整 DMG](docs/releases/v1.8.12.md)，等待现有安装结束，完全退出旧版后拖入原安装位置并选择替换，再从“应用程序”手动打开。v1.8.7 及后续版本可正常使用应用内更新。账号、任务记录和已下载文件无需删除。
